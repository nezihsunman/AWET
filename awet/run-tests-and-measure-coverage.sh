...
function runJavaMainClass() {
    ...
    local technique_local=$3
    java -cp $classpath \
        com.awet_integrated.comparison.$main_class_name_local \
            > $HOME/Desktop/logs"_"$main_class_name_local"_"$technique_local"_"$application_name_local.txt \
            2> $HOME/Desktop/errors"_"$main_class_name_local"_"$technique_local"_"$application_name_local.txt
}
...
function checkExtractionStrategy(){
	local extraction_strategy_local=$1
	if [[ $extraction_strategy_local != "biobjective" && $extraction_strategy_local != "coverage_driven" ]]; then
		echo Unknown extraction strategy: $extraction_strategy_local
		exit 1
	fi
}
...
function checkTechnique(){
	local technique_local=$1
	if [[ $technique_local != "awet" && $technique_local != "crawljax" && $technique_local != "atusa" ]]; then
		echo Unknown technique name: $technique_local
		exit 1
	fi
}
...
function setHeadless(){
    local headless_local=$1
    if [[ $os == "Darwin" ]]; then
        sed -i "" "s/headless=.*/headless=$headless_local/g" $properties_file
    else
        sed -i "s/headless=.*/headless=$headless_local/g" $properties_file
    fi
}
...
function setTestSuitePath() {
    ...
    local test_suite_path_local=
    if [[ $technique_local == "atusa" ]]; then
        k_local=$(grep -i "k_shortest_path=" $properties_file | cut -d'=' -f2)
        test_suite_path_local="$HOME/workspace/AWET/awet/applications/$application_name_local/testsuite-$application_name_local/src/main/java/tests/AtusaGeneratedTestSuite$element_strategy_local"K$k_local
    elif [[ $technique_local == "awet" || $technique_local == "crawljax" ]]; then
        test_suite_path_local="$HOME/workspace/AWET/awet/applications/$application_name_local/testsuite-$application_name_local/src/main/java/tests/GeneratedTestSuite$element_strategy_local"
    else
        echo Unknown technique: $technique_local
        exit 1
    fi
    if [[ $coverage == "true" ]]; then
        test_suite_path_local="$test_suite_path_local"Coverage.java
    else
        test_suite_path_local="$test_suite_path_local".java
    fi
...
function setCoverageReportDirectoryPath() {
    local application_name_local=$1
    local technique_local=$2
    local coverage_reports_directory_path_local="$HOME/workspace/AWET/awet/applications/$application_name_local/testsuite-$application_name_local/all-coverage-reports"
	if [[ $os == "Darwin" ]]; then
...
function setSuiteExecutionResultByteString() {
    local suite_execution_result_byte_string_local=$1
	if [[ $os == "Darwin" ]]; then
        sed -i "" "s/suite_execution_result_byte_string=.*/suite_execution_result_byte_string=$suite_execution_result_byte_string_local/g" $properties_file
    else
        sed -i "s/suite_execution_result_byte_string=.*/suite_execution_result_byte_string=$suite_execution_result_byte_string_local/g" $properties_file
    fi
}
...
function setSolutionByteString() {
    local solution_byte_string_local=$1
	if [[ $os == "Darwin" ]]; then
        sed -i "" "s/solution_byte_string=.*/solution_byte_string=$solution_byte_string_local/g" $properties_file
    else
        sed -i "s/solution_byte_string=.*/solution_byte_string=$solution_byte_string_local/g" $properties_file
    fi
}
...
function setTechnique() {
    local technique_local=$1
	if [[ $os == "Darwin" ]]; then
        sed -i "" "s/technique=.*/technique=$technique_local/g" $properties_file
    else
        sed -i "s/technique=.*/technique=$technique_local/g" $properties_file
    fi
}
...
if test $# -lt 3 ; then echo 'ARGS: application_name (petclinic|splittypie|retroboard|phoenix|dimeshift|ecommerce), headless (true|false), technique (awet|crawljax|atusa), extraction_strategy (biobjective|coverage_driven --> only when awet is chosen as technique)' ; exit 1 ; fi

application_name=$1
headless=$2
technique=$3
extraction_strategy=$4
...
if [[ $technique == "awet" ]]; then
	checkExtractionStrategy $extraction_strategy
fi

setApplicationName $application_name
setCoverageReportDirectoryPath $application_name
setHeadless $headless
element_strategy=$(setElementStrategy $application_name)
if [[ $technique == "atusa" ]]; then
    echo Generate and compile atusa tests...
    ./generate-atusa-tests.sh $application_name
fi
coverage=false
setTestSuitePath $application_name $element_strategy $technique $coverage
setTechnique $technique

if [[ $technique == "awet" ]]; then
    if [[ ! -f  $HOME/Desktop/logs"_"MinimizeTestSuite"_"$extraction_strategy"_"$application_name.txt ]]; then
        echo File "$HOME/Desktop/logs"_"MinimizeTestSuite"_"$extraction_strategy"_"$application_name.txt" does not exist
        echo "Run run-minimization.sh for $extraction_strategy" extraction strategy
        exit 1
    fi
    solution_byte_string=$(grep -i "\- Solution as byte string: " $HOME/Desktop/logs"_"MinimizeTestSuite"_"$extraction_strategy"_"$application_name.txt \
        | awk -F'string: ' '{print $2}' | awk '{$1=$1;print}') # last awk to remove leading and trailing spaces
    setSolutionByteString "$solution_byte_string"
fi

./setup-project-classpath.sh $application_name
runContainer $application_name $current_date
main_class_name=RunTests
runJavaMainClass $application_name $main_class_name $technique
stopContainer $application_name $current_date

suite_execution_result_byte_string=$(grep -i "\- Suite execution result as byte string: " $HOME/Desktop/logs"_"$main_class_name"_"$technique"_"$application_name.txt \
    | awk -F'string: ' '{print $2}' | awk '{$1=$1;print}') # last awk to remove leading and trailing spaces
setSuiteExecutionResultByteString "$suite_execution_result_byte_string"
coverage=true
setTestSuitePath $application_name $element_strategy $technique $coverage

runContainer $application_name $current_date
main_class_name=MeasureCoverageOfTests
runJavaMainClass $application_name $main_class_name $technique
stopContainer $application_name $current_date

# rename coverage reports directory
test_suite_directory_path="$HOME/workspace/AWET/awet/applications/$application_name/testsuite-$application_name"
if [[ $technique == "atusa" ]]; then
    k_value=$(grep -i "k_shortest_path=" $properties_file | cut -d'=' -f2)
    mv $test_suite_directory_path/coverage-reports $test_suite_directory_path/$technique-coverage-reports-k-$k_value
else
    mv $test_suite_directory_path/coverage-reports $test_suite_directory_path/$technique-coverage-reports
fi