...
public class RemoteJUnitCore {
    ...
    private interface ParsingInterface {
        ...
        @Option(longName = { "stop-execution-first-failure" }, defaultValue = "false")
        String stopExecutionFirstFailure();
    }
            ...
            try (Socket socket = new Socket("localhost", cli.getPort());
                ...
                if(!testsPath.equals("none") && !mvnPackageStructure.equals("none")){
                    ...
                    projectPackageStructure = testsPath.substring(indexOfMvnPackageStructure
                            + charactersInMvnPackageStructure + 1); // +1 is the '/' character

                    testSuiteClass = Class.forName(getTestSuiteClassName(projectPackageStructure));
...
//                    Request request = Request.method(testSuiteClass, test);
//                    Result r = core.run(request);
//                    results.add(r);
//                }

                Result r = core.run(new SubsetOfTestsTestRunner(testSuiteClass, testList, cli.stopExecutionFirstFailure()));
                results.add(r);
...
//                    objectOutputStream.writeObject(f.getDescription().getTestClass().getSimpleName());
                    // test class
                    objectOutputStream.writeObject(f.getDescription().getMethodName());
    ...
    private static String getTestSuiteClassName(String projectPackageStructure){
        int indexOfFileExtension = projectPackageStructure.lastIndexOf(".");
        return projectPackageStructure.substring(0, indexOfFileExtension)
                .replaceAll("/", ".");
    }
