...
function runJavaMainClass() {
    ...
    java -cp $classpath \
        com.awet_integrated.filtering.coverage.$main_class_name_local \
...
function setCoverageDrivenTestSelectionByteString() {
    local coverage_driven_test_selection_byte_string_local=$1
	if [[ $os == "Darwin" ]]; then
        sed -i "" "s/coverage_driven_test_selection_byte_string=.*/coverage_driven_test_selection_byte_string=$coverage_driven_test_selection_byte_string_local/g" $properties_file
    else
        sed -i "s/coverage_driven_test_selection_byte_string=.*/coverage_driven_test_selection_byte_string=$coverage_driven_test_selection_byte_string_local/g" $properties_file
    fi
}
...
main_class_name=CoverageDrivenTestSelection
runJavaMainClass $application_name $main_class_name

selection_test_suite_byte_string=$(grep -i "Solution byte string: " $HOME/Desktop/logs"_"$main_class_name"_"$application_name.txt \
    | awk -F'string: ' '{print $2}' | awk '{$1=$1;print}') # last awk to remove leading and trailing spaces
setCoverageDrivenTestSelectionByteString "$selection_test_suite_byte_string"

./setup-project-classpath.sh $application_name
runContainer $application_name $current_date
main_class_name=FixSelectedTestSuite
runJavaMainClass $application_name $main_class_name
stopContainer $application_name $current_date

selection_test_suite_byte_string=$(grep -i "\- Final filtered test suite: " $HOME/Desktop/logs"_"$main_class_name"_"$application_name.txt \
    | awk -F'suite: ' '{print $2}' | awk '{$1=$1;print}') # last awk to remove leading and trailing spaces
setCoverageDrivenTestSelectionByteString "$selection_test_suite_byte_string"