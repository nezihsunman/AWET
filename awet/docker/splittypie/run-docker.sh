...
# example: ./run-docker.sh -p yes -a 4000 -n splittypie_5
...
if [[ $PRODUCTION == "yes"  ]]; then
	...
	docker run -it --workdir=/home/splittypie --name=$CONTAINER_NAME --expose 4200 -p $PORT_APP:4200 -d --entrypoint ./run-services-docker.sh dockercontainervm/splittypie:crawler bash
	# docker exec --workdir=/home/splittypie -d $CONTAINER_NAME /bin/bash ./run-services-docker.sh
else
	if [[ $CONTAINER_NAME != "default" ]]; then
		docker run -it --workdir=/home/splittypie --name=$CONTAINER_NAME --expose 4200 -p $PORT_APP:4200 dockercontainervm/splittypie:crawler bash
	else
		docker run -it --workdir=/home/splittypie --expose 4200 -p $PORT_APP:4200 dockercontainervm/splittypie:crawler bash		
	fi
fi
