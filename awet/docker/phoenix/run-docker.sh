...
# example: ./run-docker.sh -p yes -a 4000 -d 5432 -n phoenix
...
if [[ $PRODUCTION == "yes"  ]]; then
	...
	docker run -it --workdir=/home/phoenix-trello --name=$CONTAINER_NAME --expose 4000 --expose 5432 -p $PORT_APP:4000 -p $PORT_DB:5432 --env PATH=/root/.kiex/elixirs/elixir-1.3.1/bin:/root/.kiex/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin -d --entrypoint ./run-services-docker.sh dockercontainervm/phoenix-trello:non-empty-db bash
	...
	if [[ $CONTAINER_NAME != "default" ]]; then
		docker run -it --workdir=/home/phoenix-trello --name=$CONTAINER_NAME --expose 4000 --expose 5432 -p $PORT_APP:4000 -p $PORT_DB:5432 dockercontainervm/phoenix-trello:non-empty-db bash
	else
		docker run -it --workdir=/home/phoenix-trello --expose 4000 --expose 5432 -p $PORT_APP:4000 -p $PORT_DB:5432 dockercontainervm/phoenix-trello:non-empty-db bash		
	fi
fi
