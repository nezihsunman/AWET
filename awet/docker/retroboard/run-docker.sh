...
# ./run-docker.sh -p yes -a 4000 -n retroboard-docker
...
if [[ $PRODUCTION == "yes"  ]]; then
	...
	docker run -it --workdir=/home/retro-board --name=$CONTAINER_NAME --expose 8080 -p $PORT_APP:8080 -d --entrypoint ./run-services-docker.sh dockercontainervm/retroboard:dev bash
	#docker exec --workdir=/home/retro-board -d $CONTAINER_NAME /bin/bash ./run-services-docker.sh
else
	if [[ $CONTAINER_NAME != "default" ]]; then
		docker run -it --workdir=/home/retro-board --name=$CONTAINER_NAME --expose 8080 -p $PORT_APP:8080 dockercontainervm/retroboard:dev bash
	else
		docker run -it --workdir=/home/retro-board --expose 8080 -p $PORT_APP:8080 dockercontainervm/retroboard:dev bash		
	fi
fi
