...
# run test suite (application has to be active on http://localhost:4000)
cp=$(mvn dependency:build-classpath | grep ".jar:")

if [[ $coverage == "true" ]]; then
    java -cp "$cp:./target/classes" org.junit.runner.JUnitCore tests.GeneratedTestSuiteFiredCoverage
else
    java -cp "$cp:./target/classes" org.junit.runner.JUnitCore tests.GeneratedTestSuiteFired
fi