...
public class BasePageObject {
    ...
    public BasePageObject(WebDriver driver) {
        ...
        FileUtils.deleteScreenshotsNonRecursively(new File(Properties.user_dir + "/src/main/resources"));
    }
    ...
    public void takeScreenshot(String testName, int num) {
        long start = System.currentTimeMillis();
        File screenshotFile = ((TakesScreenshot) this.driver).getScreenshotAs(OutputType.FILE);
        try {
            FileUtils.copyFile(screenshotFile, new File(Properties.user_dir
                    + "/src/main/resources/" + testName + "-" + num + ".png"));
            System.out.println("Time taking screenshot: "
    ...
    public void saveButtonClick(By by) {
        this.click(by);
        int timeout = 500;
        if(!this.waitForElementBeingPresentOnPage(By.xpath("//div[contains(@class,\"liquid-child\")]"),
                timeout, TimeUnit.MILLISECONDS)){
            if(this.isElementPresentOnPage(by)){
                // if it is on the page repeat the process
                this.saveButtonClick(by);
            }
            /*else{
    ...
    public void clickSettleUp(By by) {
        this.builder.moveToElement(this.driver.findElement(by)).click().perform();
        int timeout = 500;
        if(!this.waitForElementBeingPresentOnPage(By.xpath("//h4[text()=\"Settle Up!\"]"),
                timeout, TimeUnit.MILLISECONDS)){
            if(this.isElementPresentOnPage(by)){
                // if it is on the page repeat the process
                this.clickSettleUp(by);
            }
            /*else{
                throw new IllegalStateException("save button not present in the page");
            }*/
