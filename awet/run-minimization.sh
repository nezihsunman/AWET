...
function runJavaMainClass() {
    ...
    local extraction_strategy_local=$3
    java -cp $classpath \
        com.awet_integrated.minimization.$main_class_name_local \
            > $HOME/Desktop/logs"_"$main_class_name_local"_"$extraction_strategy_local"_"$application_name_local.txt \
            2> $HOME/Desktop/errors"_"$main_class_name_local"_"$extraction_strategy_local"_"$application_name_local.txt
}
...
function setValidatedGraphPath() {
    local application_name_local=$1
    local validation_results_directory_local=$2
    local extraction_strategy=$3
    local validated_graph_path_local=
    if [[ $extraction_strategy == "biobjective" ]]; then
        validated_graph_path_local="$validation_results_directory_local/dependency-graph-$extraction_strategy-final-recover-missed-$application_name_local.txt"
    elif [[ $extraction_strategy == "coverage_driven" ]]; then
        validated_graph_path_local="$validation_results_directory_local/dependency-graph-$extraction_strategy-final-$application_name_local.txt"
    else
        echo Unknown exctraction strategy: $extraction_strategy
        exit 1
    fi
	if [[ $os == "Darwin" ]]; then
        sed -i "" "s%validated_graph_path=.*%validated_graph_path=$validated_graph_path_local%g" $properties_file
    else
        sed -i "s%validated_graph_path=.*%validated_graph_path=$validated_graph_path_local%g" $properties_file
    fi
}
...
if test $# -lt 3 ; then echo 'ARGS: application_name (petclinic|splittypie|retroboard|phoenix|dimeshift|ecommerce), headless (true|false), extraction_strategy (biobjective|coverage_driven)' ; exit 1 ; fi
...
validation_results_folder=$HOME/Desktop/$application_name-validation-$extraction_strategy
validated_graph_path=
if [[ ! -d $validation_results_folder ]]; then
    echo Validation results folder $validation_results_folder not present!
    exit 1
fi
setValidatedGraphPath $application_name $validation_results_folder $extraction_strategy

main_class_name=MinimizeTestSuite
runJavaMainClass $application_name $main_class_name $extraction_strategy

solution_byte_string=$(grep -i "\- Solution as byte string: " $HOME/Desktop/logs"_"$main_class_name"_"$extraction_strategy"_"$application_name.txt \
...
main_class_name=ExecuteMinimizedTestSuite
runJavaMainClass $application_name $main_class_name $extraction_strategy
