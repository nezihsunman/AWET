...
function setIndirectDependencies() {
    local consider_indirect_dependencies_local=$1
	if [[ $os == "Darwin" ]]; then
        sed -i "" "s/consider_indirect_dependencies=.*/consider_indirect_dependencies=$consider_indirect_dependencies_local/g" $properties_file
    else
        sed -i "s/consider_indirect_dependencies=.*/consider_indirect_dependencies=$consider_indirect_dependencies_local/g" $properties_file
    fi
}

function setBiobjectiveFilterResult() {
    local dependency_graph_string_local=$1
	if [[ $os == "Darwin" ]]; then
        sed -i "" "s/biobjective_dependency_graph_byte_string=.*/biobjective_dependency_graph_byte_string=$dependency_graph_string_local/g" $properties_file
    else
        sed -i "s/biobjective_dependency_graph_byte_string=.*/biobjective_dependency_graph_byte_string=$dependency_graph_string_local/g" $properties_file
    fi
}
...
if test $# -lt 2 ; then echo 'ARGS: application_name (petclinic|splittypie|retroboard|phoenix|dimeshift|ecommerce), plot_pareto_front (true|false), pop_size (num), max_evaluations (num)' ; exit 1 ; fi
...
checkBoolean $plot_pareto_front
...
setIndirectDependencies $consider_indirect_dependencies

main_class_name=ComputeParetoFront
java -cp $classpath com.awet_integrated_integrated.filtering.biobjective.$main_class_name \
    $pop_size $max_evaluations > $HOME/Desktop/logs"_"$main_class_name"_"$application_name.txt \
            2> $HOME/Desktop/errors"_"$main_class_name"_"$application_name.txt

mkdir -p ~/Desktop/$application_name-biobjective-filter

mv FUN.tsv FUN-$application_name.tsv
mv FUN-$application_name.tsv ~/Desktop/$application_name-biobjective-filter
mv VAR.tsv VAR-$application_name.tsv
mv VAR-$application_name.tsv ~/Desktop/$application_name-biobjective-filter

if [[ $plot_pareto_front == "true" ]]; then
    Rscript plot-biobjective-pareto.R ~/Desktop/$application_name-biobjective-filter/FUN-$application_name.tsv
fi

main_class_name=ComputeParetoBestSolution
java -cp $classpath com.awet_integrated.filtering.biobjective.$main_class_name \
    ~/Desktop/$application_name-biobjective-filter > $HOME/Desktop/logs"_"$main_class_name"_"$application_name.txt \
            2> $HOME/Desktop/errors"_"$main_class_name"_"$application_name.txt

dependency_graph_string=$(grep -i "Correspondent solution: " $HOME/Desktop/logs"_"$main_class_name"_"$application_name.txt \
    | awk -F'solution: ' '{print $2}' | awk '{$1=$1;print}') # last awk to remove leading and trailing spaces
setBiobjectiveFilterResult "$dependency_graph_string"
