...
function runJavaMainClass() {
    ...
    java -cp $classpath \
        com.awet_integrated.suitegenerator.$main_class_name_local
}
...
function compileAndRunGeneratedTests(){
    local application_name_local=$1
    local element_strategy_local=$2
    local compile_local=$3
    local measure_coverage_local=$4
    local compile_script_folder=./applications/$application_name_local/testsuite-$application_name_local
    if [[ -e $compile_script_folder ]]; then
        if [[ -d $compile_script_folder ]]; then
            if [[ -f $compile_script_folder/compile-and-run-$element_strategy_local.sh ]]; then
                local pwd=$(pwd)
                cd $compile_script_folder
                chmod +x compile-and-run-$element_strategy_local.sh
                ./compile-and-run-${element_strategy_local}.sh $compile_local $measure_coverage_local
                cd $pwd
            else
                echo $compile_script_folder/compile-and-run-$element_strategy_local.sh does not exist
                exit 1
            fi
        else
            echo $compile_script_folder is not a directory
            exit 1
        fi
    else
        echo $compile_script_folder path does not exists
        exit 1
    fi
}
