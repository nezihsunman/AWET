...
public final class WebDriverBackedEmbeddedBrowser implements EmbeddedBrowser {
	...
	public void goToUrl(URI url) {
		...
		FSUtils.writeLineOnFile("driver.get(\"" + url.toString() + "\");");
		FSUtils.writeLineOnFile("Thread.sleep(" + this.crawlWaitReload + ");");
	}

	@Override
	public void crawlPathEnd(String separator) {
		FSUtils.writeLineOnFile(separator);
	}
	...
	public void handlePopups() {
		...
		if (ExpectedConditions.alertIsPresent().apply(browser) != null) {
			try {
				...
				FSUtils.writeLineOnFile("driver.switchTo().alert().accept();");
				LOGGER.info("Alert accepted");
			} catch (Exception e) {
				FSUtils.writeExceptionOnFile("Handling of PopUp windows failed");
	...
	public boolean input(Identification identification, String text) {
		try {
			...
			if (field != null) {
				...
				FSUtils.writeLineOnFile("driver.findElement("
						+ identification.getWebDriverByString() + ").clear();");
				FSUtils.writeLineOnFile("driver.findElement("
						+ identification.getWebDriverByString() + ").sendKeys(\"" + text + "\");");
		...
		try {
			...
			if (eventable.getRelatedFrame() != null && !eventable.getRelatedFrame().equals("")) {
				...
				try {
					...
					FSUtils.writeExceptionOnFile("Frame not found");
			...
			if (webElement != null) {
				...
				if(result) {
					FSUtils.writeLineOnFile("driver.findElement("
							+ eventable.getIdentification().getWebDriverByString() + ").click();");
					FSUtils.writeLineOnFile("Thread.sleep(" + this.crawlWaitEvent + ");");
				}
			}

			if (handleChanged) {
				FSUtils.writeLineOnFile("driver.switchTo().defaultContent();");
	...
	public Object executeJavaScript(String code) throws CrawljaxException {
		try {
			...
			FSUtils.writeLineOnFile("js.executeScript(\"" + code + "\");");
			return result;
		} catch (WebDriverException e) {
			throwIfConnectionException(e);
		...
		try {
			...
			for (String handle : browser.getWindowHandles()) {
				if (!handle.equals(browser.getWindowHandle())) {
					...
					FSUtils.writeLineOnFile("driver.switchTo().window(" + handle + ");");
					LOGGER.debug("Closing other window with title \"{}\"", browser.getTitle());
					FSUtils.writeLineOnFile("driver.close();");
					browser.close();
					FSUtils.writeLineOnFile("driver.switchTo().window(" + current + ");");
					browser.switchTo().window(current);
				}
			}
		} catch (UnhandledAlertException e) {
			FSUtils.writeExceptionOnFile("While closing the window, an alert got ignored");
			LOGGER.warn("While closing the window, an alert got ignored: {}", e.getMessage());
		} catch (WebDriverException e) {
			FSUtils.writeExceptionOnFile(e.getMessage().split("\n")[0]);
			throw wrapWebDriverExceptionIfConnectionException(e);
		}
	}
	...
	private void switchToFrame(String frameIdentification) throws NoSuchFrameException {
		...
		if (frameIdentification.contains(".")) {
			...
			for (String frameId : frames) {
				...
				FSUtils.writeLineOnFile("driver.switchTo().frame(" + frameId + ");");
			}

		} else {
			browser.switchTo().frame(frameIdentification);
			FSUtils.writeLineOnFile("driver.switchTo().frame(" + frameIdentification + ");");
		}
