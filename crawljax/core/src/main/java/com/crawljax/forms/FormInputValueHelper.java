...
public final class FormInputValueHelper {
	...
	private final Set<CrawlElement> formSubmitButtons = new HashSet<>();
	...
	private boolean disableIdAndNameIdentification;
	...
	public static synchronized FormInputValueHelper getInstance(
			InputSpecification inputSpecification, FormFillMode formFillMode, boolean disableIdAndNameIdentification) {
		if (instance == null)
			instance = new FormInputValueHelper(inputSpecification,
					formFillMode, disableIdAndNameIdentification);
		return instance;
	}
	...
	private FormInputValueHelper(InputSpecification inputSpecification,
			FormFillMode formFillMode, boolean disableIdAndNameIdentification) {
		...
		for (Form form : inputSpecification.getForms()) {
			this.formSubmitButtons.add(form.getCrawlElement());
			for (FormInput input : form.getFormInputs()) {
				this.formInputs.put(input.getIdentification(), input);
			}
		}

		this.disableIdAndNameIdentification = disableIdAndNameIdentification;

	}

	public Set<CrawlElement> getFormSubmitButtons() {
		return new HashSet<>(this.formSubmitButtons);
	}
	...
	private Identification getIdentification(Node element) {
		...
		if(!this.disableIdAndNameIdentification) {
	...
	private FormInput formInputMatchingNode(Node element) {
		...
		// changed in absoluteXPathExpression
//		String xpathExpr = XPathHelper.getXPathExpression(element);
		String xpathExpr = XPathHelper.getAbsoluteXPathExpression(element);
		if (xpathExpr != null && !xpathExpr.equals("")) {
