...
public class Crawler {
	...
	private final String crawlPathSeparator;
	...
	public void reset() {
		...
		System.out.println("Go to url: " + url.toString());
		browser.goToUrl(url);
...
//		System.out.println("Resetting the crawler and going to state " + crawlTask.getName());
		...
		try {
			...
			if (!reachable) {
				LOG.info("state unreachable: Removing from candidate actions {}",
						crawlTask.getName());
				System.out.println("State unreachable: removing from candidate actions " + crawlTask.getName());
	...
	private boolean reachFromHome(StateVertex crawlTask) {
		...
		try {
			...
			System.out.println(crawlTask.getName() + " no path from "
					+ stateMachine.getCurrentState().getName());
			path = null;
		}
		if (path != null) {
...
//				Ex.printStackTrace();
				...
				System.out.println(crawlTask.getName() + " no path from " + onURL.getName());
				path = null;
			}
			if (path != null) {
				try {
...
//					reset();
	...
	private ImmutableList<Eventable> shortestPathTo(StateVertex crawlTask) {
		...
		if (currState != null && currState != graph.getInitialState()) {
			...
			System.out.println("Initial state not the same as current state : Getting shortest path from current state");
	...
	private void follow(CrawlPath path, StateVertex targetState)
		...
		for (Eventable clickable : path) {
			...
			LOG.debug("Backtracking by executing {} on element: {}", clickable.getEventType(),
					clickable);
			System.out.println("Backtracking by executing " + clickable.getEventType()
		...
		if (!currState.equals(targetState)) {
			System.out.println("The path did not result in the desired state " + targetState
					+ " but in state " + currState.getName());
	...
	private StateVertex changeState(StateVertex targetState, Eventable clickable) {
		...
		if (!switched) {
			System.out.println("Could not switch states (" + targetState + ")");
		...
		System.out.println("Changed state to " + curState);
	...
	private void tryToFireEvent(StateVertex targetState, StateVertex curState,
		...
		if (fireEvent(clickable)) {
			...
			System.out.println("Crawl depth is now " + depth);
	...
	private void handleInputElements(List<FormInput> formInputs) {
		formHandler.handleFormElements(formInputs);
	}
	...
	private boolean isFormSubmitButton(Eventable eventable) {
		Set<CrawlElement> formSubmitButtons = formHandler.getFormSubmitButtons();
		Element element = eventable.getElement();
		for (CrawlElement formSubmitButton : formSubmitButtons) {
			if(formSubmitButton.getTagName().equalsIgnoreCase(element.getTag())) {
				String text = formSubmitButton.getText();
				if(text.equalsIgnoreCase(element.getText())) {
					return true;
				}
				String attributeName = formSubmitButton.getAttributeName();
				String attributeValue = formSubmitButton.getAttributeValue();
				if(element.getAttributeOrNull(attributeName) != null) {
					String attributeElementValue = element.getAttributeOrNull(attributeName);
					if(attributeValue.equalsIgnoreCase(attributeElementValue)) {
						return true;
					}
				}
				String xpathExpression = formSubmitButton.getWithXpathExpression();
				if(eventable.getIdentification().getHow().equals(Identification.How.xpath)) {
					if(eventable.getIdentification().getValue().equalsIgnoreCase(xpathExpression)) {
						return true;
					}
				}
			}
	...
	private boolean fireEvent(Eventable eventable) {
		...
		try {
			...
			if (crawlRules.isCrawlHiddenAnchors() && eventToFire.getElement() != null
				...
				System.out.println("Ignoring invisible element: " + eventToFire.getElement());
			}
		} catch (InterruptedException e) {
			LOG.debug("Interrupted during fire event");
			System.out.println("Interrupted during fire event");
			interruptThread();
			return false;
		}
		...
		if (isFired) {
			System.out.println("Event fired id: "
					+ CandidateElement.getElementId(eventable.getCandidateElement().getElement(),
					eventable.getCandidateElement().getIdentification()));
	...
	private boolean visitAnchorHrefIfPossible(Eventable eventable) {
		...
		if (href == null) {
			...
			System.out.println("Anchor " + element + " has no href and it is invisible so it will be ignored");
		} else {
	...
	private void crawlThroughActions() {
		System.out.println("Crawl through actions");
		...
		List<FormInput> cachedInputElements = new ArrayList<>();
		while (action != null && !interrupted) {
			...
			if (element.allConditionsSatisfied(browser)) {
				...
				// works only if there is only one eventable that is the form submission button
				if(this.handleSameFormInputsOncePerState && this.isFormSubmitButton(event)) {
					List<FormInput> inputElements = this.getInputElements(event);
					List<FormInput> newInputs = this.getNewInputs(cachedInputElements, inputElements);
				...
				System.out.println("Element " + element
						+ " not clicked because not all crawl conditions were satisfied");
			}

			if (newStateFound && stateMachine.getCurrentState().hasNearDuplicate()) {
				System.out.println("New state found, stop firing actions on state " + currentState);
				action = null;
			} else {
		...
		if (interrupted) {
			...
			System.out.println("Interrupted while firing actions. Putting back the actions on the todo list");
			if (action != null) {
	...
	private List<FormInput> getNewInputs(List<FormInput> previousInputs, List<FormInput> currentInputs) {
		List<FormInput> newInputs = new ArrayList<>();
		for (FormInput currentInput : currentInputs) {
			if(!previousInputs.contains(currentInput)) {
				newInputs.add(currentInput);
	...
	private boolean inspectNewState(Eventable event) {
		...
		if (crawlerLeftDomain()) {
			...
			if (domChanged(event, newState)) {
				...
				System.out.println("Dom unchanged");
				return false;
			}
		}
	}
	...
	private boolean inspectNewDom(Eventable event, StateVertex newState) {
		...
		System.out.println("The DOM has changed. Event added to the crawl path");
		crawlpath.add(event);
		...
		if (isNewState) {
			...
			System.out.println("New DOM is a new state! crawl depth is now " + depth);
			if (maxDepth == depth) {
				LOG.debug("Maximum depth achieved. Not crawling this state any further");
				System.out.println("Maximum depth achieved. Not crawling this state any further");
				return true;
			} else {
				parseCurrentPageForCandidateElements();
				return true;
			...
			if (event.getId() == -1) {
				...
				for (Eventable edge : stateMachine.getStateFlowGraph().getAllEdges()) {
					if (edge.equals(event)) {
						...
						System.out.println("CrawlPath fixed !! The eventable fired was a clone of existing edge");
						break;
					}
				}
			}
			LOG.debug("New DOM is a clone state " + newState + ". Continuing in that state.");
			System.out.println("New DOM is a clone state. Continuing in that state.");
			return false;
		}
	}
	...
	private void parseCurrentPageForCandidateElements() {
		...
		System.out.println("Parsing DOM of state " + currentState.getName() + " for candidate elements");
		...
		if ("meta".equalsIgnoreCase(eventable.getElement().getTag())) {
			...
			for (Entry<String, String> e : eventable.getElement().getAttributes().entrySet()) {
				...
				try {
					...
					System.out.println("Crawler timed out while waiting for page to reload");
					interruptThread();
				}
			}
		}
	...
	private void goBackOneState() {
		...
		System.out.println("Going back one state");
...
//		reset();
		...
		if (!currentPath.get(0).getSourceStateVertex()
			...
			System.out.println("Landing Page has changed on reset trying to go back one page!! Saving that path");
			CrawlSession session = context.getSession();
	...
	public StateVertex crawlIndex() {
		...
		System.out.println("Setting up vertex of the index page");
		...
		System.out.println("Parsing the index for candidate elements");
