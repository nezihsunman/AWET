...
public class CandidateElementExtractor {
	...
	private final CrawljaxConfiguration.CandidateElementsMode candidateElementsMode;
	...
	public CandidateElementExtractor(ExtractorManager checker, @Assisted EmbeddedBrowser browser,
		...
		candidateElementsMode = config.getConsiderCandidateElementsOnce();
	}
	...
	public ImmutableList<CandidateElement> extract(StateVertex currentState)
		...
		if (!checkedElements.checkCrawlCondition(browser)) {
			...
			System.out.println("State " + currentState.getName() + " did not satisfy the CrawlConditions.");
		...
		System.out.println("Found " + results.size() + " new candidate elements to analyze. Elements: " + results);
	...
	private ImmutableList<Element> getNodeListForTagElement(Document dom,
		...
		for (int k = 0; k < nodeList.getLength(); k++) {
			...
			if(candidateElementsMode.equals(CrawljaxConfiguration.CandidateElementsMode.checked)) {
				/*
...
//				}
	...
	public boolean isVisible(Element element) {
		...
		if(!browser.getWebDriver().findElement(By.xpath(xpathExpression)).isDisplayed()){
			System.out.println("Excluded element " + element + " because not visible. Xpath: "
					+ xpathExpression);
			return false;
		}
		return true;
	...
	public boolean isElementFired(CandidateElement candidateElement){
		return checkedElements.isFired(candidateElement.getElement(), candidateElement.getIdentification());
	}
}
