...
public class CrawljaxConfiguration {
	...
	public static class CrawljaxConfigurationBuilder {
		...
		public CrawljaxConfigurationBuilder setHandleSameFormInputsOncePerState(boolean handleSameFormInputsOncePerState) {
			config.handleSameFormInputsOncePerState = handleSameFormInputsOncePerState;
			return this;
		}

		public CrawljaxConfigurationBuilder setConsiderCandidateElementsOnce(CandidateElementsMode mode){
			Preconditions.checkNotNull(mode, "Candidate elements mode cannot be null");
			config.considerCandidateElementsOnce = mode;
			return this;
		}

		public CrawljaxConfigurationBuilder setSeleniumActionsDir(String dir) {
			config.seleniumActionsDir = dir;
			Preconditions.checkArgument(new File(dir).exists(), "Selenium actions dir does not exist!");
			return this;
		}

		public CrawljaxConfigurationBuilder setSeleniumActionsFileName(String name) {
			config.seleniumActionsFileName = name;
			return this;
		}

		public CrawljaxConfigurationBuilder setCrawlPathSeparator(String separator) {
			Preconditions.checkNotNull(separator, "Crawl path separator cannot be null");
			config.crawlPathSeparator = separator;
			return this;
		}
		...
		public CrawljaxConfiguration build() {
			...
			try {
				FSUtils.seleniumActionsWriter = new PrintWriter(
						new File(config.seleniumActionsDir + File.separator + config.seleniumActionsFileName));
		...
		public CrawljaxConfiguration buildWithoutWritingLogFile() {
			config.plugins = pluginBuilder.build();
			config.crawlRules = crawlRules.build();
	...
	public enum CandidateElementsMode {
		fired, checked
	}

	private CandidateElementsMode considerCandidateElementsOnce = CandidateElementsMode.checked;

	private String crawlPathSeparator = "----";

	private String seleniumActionsDir = System.getProperty("user.home") + File.separator + "Desktop";

	private String seleniumActionsFileName = "selenium-actions";

	private boolean handleSameFormInputsOncePerState = false;
	...
	public CandidateElementsMode getConsiderCandidateElementsOnce() { return  considerCandidateElementsOnce; }
