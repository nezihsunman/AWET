...
public class UnfiredCandidateActions {
		...
		try {
			...
			if (queue == null) {
				...
				if (queue.isEmpty()) {
					...
					System.out.println("All actions polled for state " + state.getName());
					...
					System.out.println("There are now " + cache.size() + " states with unfinished actions");
				}
				return action;
			}
		} finally {
	...
	public void purgeActionForState(StateVertex crawlTask, CandidateElement actionToPurge) {
		Lock lock = locks.get(crawlTask.getId());
		try {
			lock.lock();
			LOG.debug("Removing tasks for target state {}", crawlTask.getName());
			System.out.println("Removing task " + actionToPurge + " for target state " + crawlTask.getName());
			Queue<CandidateCrawlAction> queue = cache.get(crawlTask.getId());
			List<CandidateCrawlAction> actionsToPurge = queue.stream()
					.filter(candidateCrawlAction -> candidateCrawlAction.getCandidateElement().equals(actionToPurge))
					.collect(Collectors.toList());
			for(CandidateCrawlAction candidateCrawlAction: actionsToPurge){
				System.out.println("Removing action " + candidateCrawlAction + " from state " + crawlTask.getName());
				if(queue.remove(candidateCrawlAction)){
					unfiredActionsCount.dec();
				}
			}
			cache.put(crawlTask.getId(), queue);
		} finally {
			lock.unlock();
