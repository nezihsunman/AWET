...
public class CandidateElement {
	...
	public static String getElementId(Element element, Identification identification) {
		ImmutableSet<String> exclude = ImmutableSet.of("atusa");

		StringBuilder result = new StringBuilder();
		if (element != null) {
			result.append(element.getNodeName()).append(": ");

		}
		result.append(DomUtils.getElementAttributes(element, exclude)).append(' ')
