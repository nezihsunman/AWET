	...
	private void setMaximumCrawlTimeIfNeeded() {
		...
		executor.submit(() -> {
			try {
				...
				System.out.println("Waiting " + maximumCrawlTime + " before killing the crawler");
				Thread.sleep(maximumCrawlTime);
				LOG.info("Time is up! Shutting down...");
				System.out.println("Time is up! Shutting down...");
				exitNotifier.signalTimeIsUp();
			} catch (InterruptedException e) {
				LOG.debug("Crawler finished before maximum crawl time exceeded");
				System.out.println("Crawler finished before maximum crawl time exceeded");
			}
